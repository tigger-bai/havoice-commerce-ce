...
import {
  ...
  LoadingSpinner,
  ErrorAlert,
} from "@/components/ui/LoadingAndError";
import {
  ProductForm,
...
export default function EditProductPage() {
  ...
  const fetchProduct = useCallback(async () => {
    ...
    try {
      ...
      setData({
        ...
        cost:
          p.cost === null || p.cost === undefined ? null : Number(p.cost) || 0,
        sku: p.sku ?? "",
        barcode: p.barcode ?? "",
        ...
        detailBlocks: Array.isArray(p.detailBlocks) ? p.detailBlocks : [],
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        title="編輯商品"
        description={productName ? `正在編輯：${productName}` : "修改商品資訊"}
      />
