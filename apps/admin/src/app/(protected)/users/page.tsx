...
import { UserStatusInlineEdit } from '@/components/users/UserInlineEdit';
 ...
 * - 專業 DataTable：姓名 / Email / 角色 / 狀態 / 註冊時間 / 操作
 * - 行內快速編輯：Role 與 Status 直接以下拉切換，PATCH 樂觀更新
 * - 右上「+ 新增會員」；每列操作含「編輯」「刪除」(ConfirmDialog 軟刪除)
 * - 分頁 + 關鍵字搜尋（name / email）
 * - 防禦：所有數值經 safeNumber 轉換，API 錯誤以 ErrorAlert 呈現，畫面不崩潰
 */
...
function UsersContent() {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || !json.success) {
        throw new Error(json?.error?.message || '載入會員失敗');
      }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="商城會員"
        description={`共 ${total} 位可登入前台商城的會員`}
        actions={
          <Link
            href="/users/new"
      ...
      <p className="text-xs text-gray-400">
        提示：本頁僅顯示一般會員（USER）。狀態可直接於欄位中以下拉切換即時儲存；管理人員帳號請至「系統帳號管理」設定。
      </p>
      ...
      <ConfirmDialog
        ...
        danger
        title="確認刪除會員"
        description={deleteTarget ? `確定要刪除「${deleteTarget.name || deleteTarget.email}」嗎？此操作為軟刪除（保留歷史訂單關聯），該帳號將無法再登入。` : ''}
        confirmText="刪除"
        cancelText="取消"
        loading={isDeleting}
